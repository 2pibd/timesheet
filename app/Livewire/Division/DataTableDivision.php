...
class DataTableDivision extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        if (!empty($this->search)) {
            $query->where('division', 'like', '%' . $this->search . '%') ;
        }
        ...
        return view('livewire.division.data-table-division' ,   [ 'data' => $data  ]);
    ...
    public function exportToExcel(Request $request)
    {
        ...
        return Excel::download(new EmailTemplateExport($filters), time().'_division.xlsx');
    }


    public function exportToPDF(Request $request){

        $data = division::all(); // Fetch all data
        $pdf = Pdf::loadView('admin.division.export', compact('data'));
