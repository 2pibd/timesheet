...
namespace App\Livewire\LeavingDetails;
...
class DataTableLeavingDetails extends Component
{
    use WithPagination;
    ...
    public $sortField = 'personal_ref'; // Default sort field
    ...
    public function render()
    {
        ...
        $query = leaving_detail::query();

        if (!empty($this->search)) {
            $query->where('personal_ref', 'like', '%' . $this->search . '%')
                ->orWhere('leaving_date', 'like', '%' . $this->search . '%');
        }
        ...
        return view('livewire.leaving-details.data-table-leaving-details' ,   [ 'data' => $data  ]);
