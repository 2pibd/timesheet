            ...
            <ul class="dropdown-menu" aria-labelledby="columnVisibilityDropdown">
                <li>
                    <label class="dropdown-item">
                        <input type="checkbox" class="form-check-input" id="colTplname" checked onclick="toggleColumn('tplname')"> Template Name
                    </label>
                </li>
                <li>
                    <label class="dropdown-item">
                        <input type="checkbox" class="form-check-input" id="colSubject" checked onclick="toggleColumn('subject')"> Subject
                    </label>
                </li>
                <li>
                    <label class="dropdown-item">
                        <input type="checkbox" class="form-check-input" id="colTemplateType" checked onclick="toggleColumn('template_type_id')"> Template Type
                    </label>
                </li>
                <li>
                    <label class="dropdown-item">
                        <input type="checkbox" class="form-check-input" id="colStatus" checked onclick="toggleColumn('status')"> Status
                    </label>
                </li>
            </ul>
        </div>
...
<div class="table-bordered table-responsive printable-section">
    ...
    <table class="table table-striped">
        ...
        <tr>
            <th>#</th>
            <th data-column="tplname">
                <button wire:click="sortBy('tplname')" class="btn btn-link p-0 d-flex justify-content-between align-items-center w-100">
                    <span class="text-start">Template Name</span>
            ...
            <th  data-column="subject">
                ...
                <button wire:click="sortBy('subject')" class="btn btn-link p-0 d-flex justify-content-between align-items-center w-100">
                    <span class="text-start">Subject</span>
                    <span class="sort-icons">
                       <i class="fa fa-sort-up {{ $sortField === 'subject' && $sortDirection === 'asc' ? 'active-icon' : 'light-icon' }}"></i>
                       <i class="fa fa-sort-down {{ $sortField === 'subject' && $sortDirection === 'desc' ? 'active-icon' : 'light-icon' }}"></i>
                    </span>
                </button>
            </th>
            <th data-column="template_type_id">
                <button wire:click="sortBy('template_type_id')" class="btn btn-link p-0 d-flex justify-content-between align-items-center w-100">
                    <span class="text-start">Template Type</span>
                    <span class="sort-icons">
                       <i class="fa fa-sort-up {{ $sortField === 'template_type_id' && $sortDirection === 'asc' ? 'active-icon' : 'light-icon' }}"></i>
                       <i class="fa fa-sort-down {{ $sortField === 'template_type_id' && $sortDirection === 'desc' ? 'active-icon' : 'light-icon' }}"></i>
                    </span>
                </button>
            </th>
        ...
        @forelse ($data as $item)
            <tr>
                ...
                <td data-column="tplname">{{ $item->tplname }}</td>
                <td data-column="subject">{{ $item->subject }}</td>
                <td data-column="template_type_id">{{ $item->template_type->type ?? '' }}</td>
                        ...
                        <li class="list-inline-item no-print">
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu dropdown-menu-end">
                                    ...
                                    <li><a href="{{ route('email_template.show', $item->id) }}" class="dropdown-item view-item-btn"
                                                        title="View"><i class="ri-eye-fill align-bottom me-2 text-muted"></i> View</a></li>
                                     @endcan
                                        @can('update-email_template')
                                  <li><a href="{{ route('email_template.edit', $item->id) }}" class="dropdown-item edit-item-btn"
                                    ...
                                    <li>
                                    <form method="POST" action="{{ route('email_template.destroy', $item->id) }}"
                                          accept-charset="UTF-8" style="display:inline">
                                        {{ method_field('DELETE') }}
                                        @csrf()
                                        <button type="submit"
                                                class="dropdown-item  btn-link text-black btn-xs"
                                                title="Delete email_template"
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        Livewire.on('print-email-template', function () {
