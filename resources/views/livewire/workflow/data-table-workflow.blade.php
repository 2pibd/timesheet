        ...
        <table class="table table-striped">
            ...
            <tr>
                ...
                <th  data-column="external_ref">
                    ...
                    <button wire:click="sortBy('escalation_frequency_id')" class="btn btn-link p-0 d-flex justify-content-between align-items-center w-100">
                        <span class="text-start">Description</span>
                        <span class="sort-icons">
                       <i class="fa fa-sort-up {{ $sortField === 'escalation_frequency_id' && $sortDirection === 'asc' ? 'active-icon' : 'light-icon' }}"></i>
                       <i class="fa fa-sort-down {{ $sortField === 'escalation_frequency_id' && $sortDirection === 'desc' ? 'active-icon' : 'light-icon' }}"></i>
                    </span>
                    </button>
                </th>
            ...
            @forelse ($data as $item)
                <tr>
                    ...
                    <td data-column="name">{{ $item->description }}</td>
                    <td data-column="description">{{ $item->escalation_frequency->description ?? '' }}</td>
                            ...
                            <li class="list-inline-item no-print">
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        @can('view-workflow')
                                            <li><a href="{{ route('workflow.show', $item->id) }}" class="dropdown-item view-item-btn"
                                                   title="View"><i class="ri-eye-fill align-bottom me-2 text-muted"></i> View</a></li>
                                        @endcan
                                        @can('update-workflow')
                                            <li><a href="{{ route('workflow.edit', $item->id) }}" class="dropdown-item edit-item-btn"
                                        ...
                                        @can('delete-workflow')
                                            <li>
                                                <form method="POST" action="{{ route('workflow.destroy', $item->id) }}"
    ...
    <script>
        ...
        $(function () {
            ...
            $(document).on('click', '.updateFlag', function () {
                ...
                $.ajax({
                    url: "{{URL::to('admin/update-workflow')}}",
                    dataType: "JSON",
                    type: 'POST',
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        Livewire.on('print-workflow', function () {
